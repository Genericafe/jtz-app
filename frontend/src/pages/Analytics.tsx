...
import { BarChart3, Route, Clock, Mountain, Flame, Activity } from 'lucide-react';
...
interface Overview { runnerId: number; nombre: string }
    ...
    <div className="p-4 lg:p-8 max-w-4xl">
      <div className="mb-5 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-gray-400 text-sm">Volumen, distribución y zonas {selectedName ? `· ${selectedName}` : ''}</p>
        </div>
      </div>
      ...
      {isCoach && overview && overview.length > 0 && (
        <div className="flex gap-2 overflow-x-auto pb-2 mb-4">
          <button onClick={() => setRunnerId(null)}
            className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-xs font-medium border ${runnerId === null ? 'bg-brand-500/15 border-brand-500/40 text-white' : 'bg-surface-700 border-white/[0.06] text-gray-400'}`}>Yo</button>
          {overview.map(o => (
            <button key={o.runnerId} onClick={() => setRunnerId(o.runnerId)}
              className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-xs font-medium border whitespace-nowrap ${runnerId === o.runnerId ? 'bg-brand-500/15 border-brand-500/40 text-white' : 'bg-surface-700 border-white/[0.06] text-gray-400'}`}>{o.nombre}</button>
          ))}
        </div>
      )}
