        ...
        <div className="card p-8">
          {step === 1 ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1.5">Ciudad <span className="text-gray-600">(opcional)</span></label>
                <input type="text" value={form.ciudad} placeholder="Ej: Tijuana, CDMX, Monterrey..."
                  onChange={(e) => setForm({ ...form, ciudad: e.target.value })}
                  className="input w-full" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1.5">Nivel actual</label>
                <select value={form.nivel} onChange={(e) => setForm({ ...form, nivel: e.target.value })} className="input w-full">
                  <option value="principiante">Principiante — estoy empezando</option>
                  <option value="intermedio">Intermedio — corro regularmente</option>
                  <option value="avanzado">Avanzado — compito en carreras</option>
                  <option value="elite">Elite — corredor de alto rendimiento</option>
                </select>
              </div>
