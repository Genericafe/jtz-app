...
// ── IMPORTANTE: la ruta estática /checkout/order DEBE estar ANTES de /checkout/:paymentId
// porque Express resuelve rutas en orden y :paymentId capturaría "order" como parámetro.

// ── Checkout para una orden de tienda (ruta estática — va primero) ─────────────
router.post('/checkout/order', authMiddleware, async (req: AuthRequest, res: Response) => {
  if (req.userRole !== 'runner') {
    return res.status(403).json({ error: 'Solo corredores pueden comprar en la tienda' });
  }
      ...
      return {
        price_data: {
          ...
          product_data: {
            name: product.nombre,
            description:
              [product.talla && `Talla ${product.talla}`, product.color]
                .filter(Boolean)
                .join(' · ') || undefined,
          },
  ...
  // Guardar session ID — operación no-bloqueante (la columna puede no existir aún en prod)
  prisma.order.update({
    where: { id: order.id },
...
// ── Checkout para un pago pendiente — membresías, cuotas, etc. (ruta con parámetro — va después) ──
...
// ── Verificar pago de membresía / cuota ───────────────────────────────────────
...
// ── Verificar pago de orden de tienda ─────────────────────────────────────────
...
// ── Webhook de Stripe — maneja membresías Y órdenes de tienda ─────────────────
